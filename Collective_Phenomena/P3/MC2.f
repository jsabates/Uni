      ...
      PROGRAM Hola
      IMPLICIT NONE
      INTEGER*4 SEED, i, u,v,NRAND, j, L,IMC,IPAS,SEED0,NSEED
      INTEGER*4 MCTOT
      PARAMETER (L=32)
      INTEGER*2 S(1:L,1:L)
      INTEGER*4 N,DE
      INTEGER*4 PBC(0:L+1)
      REAL*8 W(-8:8)
      REAL*8 TEMP,ENEBIS,DELTA,sumav,m,e,SUM,SUME,SUME2,SUMM,SUMM2,SUMAM
      REAL*8 x, suma, suma2, sigma, genrand_real2, MAG, MAGNE,ENE,ENERG
      REAL*8 VARE,VARM,MCD,MCINI
      REAL*4 TIME1,TIME2
      CHARACTER*18 NOM
      CHARACTER*30 DATE
      NAMELIST /DADES/ NOM, TEMP, NSEED, SEED0, MCTOT, MCINI, MCD

      NOM="MC-L-032-TEMP-1000"
!      SEED=234567
      TEMP=1.0D0
      NSEED=1000
      SEED0=117654
...
!      WRITE(12,*) IMC, ENE/N, ENEBIS, MAG/N
        ...
        DO IMC=1,MCTOT
          ...
          sumav=S(u,PBC(v+1))+S(u,PBC(v-1))+S(PBC(u+1),v)+S(PBC(u-1),v)
...
!      CALL SYSTEM ('gnuplot -p P3-plotconfigMAG.gnu')
