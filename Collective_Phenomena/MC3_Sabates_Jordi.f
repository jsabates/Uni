      ...
      NOM="MC-L-008"
      T1=140   !Despres dividirem entre 100 per agafar la temp correcte
      T2=340
      NSEED=200
      SEED0=117654
      MCTOT=40000
...
!codi no utilitzat per llegir les dades desde un fitxer extern
...
!BUCLE per totes les temperatures desde t1 fins a t2
      DO T=T1,T2
        CONTADOR=CONTADOR+1   !contador utilitzat per no escriure la derivada pel primer valor
        SUMEeTant=SUME        !valor de l'energia del pas anterior per calcular la derivada
        TEMP=T/100            !convertim la temperatura a decimal
        WRITE(*,*) "TEMP= ",TEMP
!millora rapidesa
        DO i=-8,8
          W(i)=exp(-i/TEMP)
...
!Calculem les variables finals

        SUME=SUME/SUM !e
        SUME2=SUME2/SUM !e2
        SUMM=SUMM/SUM !m
        SUMM2=SUMM2/SUM !m2
        SUMAM=SUMAM/SUM !abs(m)
        VARE = SUME2-SUME*SUME !variancia de e
        VARM = SUMM2-SUMM*SUMM !variancia de m
        SUS = ((SUMM2-(SUMAM**2))/(N*TEMP)) !susceptibilitat
        CAP = VARE/(N*(TEMP**2)) !capacitat calorifica
        IMANPP=sqrt(SUMM2/(N**2)) !m calculat fent larrel
        DERIV= ((SUME/N)-(SUMEeTant/N))/(0.01) !capacitat calorifica a traves de la derivada
        errore=(sqrt(VARE))/(N*sqrt(SUM)) !error per e
        errorm=(sqrt(VARM))/(N*sqrt(SUM)) !error per m, aball error de capacitat i susceptibilitat, despres escrivim al arxiu les dades
